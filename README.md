...
[查看 1.4 测试版与下载附件](https://github.com/Mj-coder686/md-app/releases/tag/v1.4.0) · [更新说明](CHANGELOG.md)
...
- **书桌式文档架**：纸白、墨黑、暖橙与封面排版，清楚显示文档类型和文件名。
- **继续阅读**：首页显示上次打开的文件与记住的位置，切换分类时保持入口原位。
- **统一阅读体验**：更新折角书页图标、阅读工具栏、编辑器、目录、搜索和三种主题。
- **字号修复**：修复设置点击导致字号异常的问题，并恢复旧设置中的无效字号。
...
自动检查覆盖旧版 MD 数据迁移、编辑保存、分类、PDF 像素/翻页/缓存/双指缩放、Word 标题/表格/图片/双指缩放/搜索、断网重开、位置与比例恢复、重复导入、损坏文件与空间不足回滚；另检查手机到宽屏布局、长名称、封面对齐、继续阅读、主题保存和字号调整。浏览器检查通过不等于 Android 真机验证，实际手机流畅度与小米警报仍待复查。
